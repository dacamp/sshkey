...
class SSHKey
  ...
  class << self
    ...
    #
    def ssh_public_key_to_ssh2_public_key(ssh_public_key, headers = {})
      raise PublicKeyError, "invalid ssh public key" unless valid_ssh_public_key?(ssh_public_key)
      ...
      if source_comment = ssh_public_key.split(source_key)[1]
        ...
        headers.merge!( { 'Comment' => source_comment.strip }) { |k, o, n| o }
      end

      if headers && (header_fields = build_ssh2_headers(headers))
        ssh2_key << header_fields
      end
    ...
    # returns ssh_type, encoded key, comment
