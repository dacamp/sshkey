...
EOF
    ...
    SSH2_PUBLIC_WITHOUT_COMMENT = <<-EOF.rstrip
---- BEGIN SSH2 PUBLIC KEY ----
AAAAB3NzaC1yc2EAAAABIwAAAQEArfTA/lKVR84IMc9ZzXOCHr8DVtR8hzWuEVHF6KElav
RHlk14g0SZu3m908Ejm/XF3EfNHjX9wN+62IMA0QBxkBMFCuLF+U/oeUs0NoDdAEKxjj4n
6lq6Ss8aLct+anMy7D1jwvOLbcwV54w1d5JDdlZVdZ6AvHm9otwJq6rNpDgdmXY4HgC2nM
...
EOF
  ...
  def test_ssh2_public_key_output
    ...
    expected4 = SSH2_PUBLIC_WITHOUT_COMMENT
    ...
    assert_equal expected4, @key_without_comment.ssh2_public_key
  end
  ...
  def test_ssh_public_key_to_ssh2_public_key
    expectation1 = "ssh-rsa #{SSH_PUBLIC_KEY1} me@example.com"
    expectation2 = "ssh-rsa #{SSH_PUBLIC_KEY2} me@example.com"
    expectation3 = "ssh-dss #{SSH_PUBLIC_KEY3} 1024-bit DSA with provided comment"
    expectation4 = "ssh-rsa #{SSH_PUBLIC_KEY1}"

    assert_equal(SSH2_PUBLIC_KEY1, SSHKey.ssh_public_key_to_ssh2_public_key(expectation1))
    assert_equal(SSH2_PUBLIC_KEY2, SSHKey.ssh_public_key_to_ssh2_public_key(expectation2))
    assert_equal(SSH2_PUBLIC_KEY3,
                 SSHKey.ssh_public_key_to_ssh2_public_key(expectation3,
                                                          { 'x-private-use-header' => 'some value that is long enough to go to wrap around to a new line.' }) )
    assert_equal(SSH2_PUBLIC_WITHOUT_COMMENT, SSHKey.ssh_public_key_to_ssh2_public_key(expectation4))
  end
